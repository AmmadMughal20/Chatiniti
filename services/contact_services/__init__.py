...
def get_contact(id):
    conn = db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, first_name, middle_name, last_name, phone, email, created_at, updated_at FROM contacts WHERE id = %s;", (id,))
...
def get_contacts(user_id):
    conn = db_connection()
    cur = conn.cursor()
    cur.execute("SELECT con.id, con.first_name, con.middle_name, con.last_name, con.phone, con.email, con.created_at, con.updated_at FROM contacts con left join user_contact uc on con.id = uc.contact_id WHERE user_id = %s;", (user_id,))
...
def remove_user_contact(user_id, contact_id):
    conn = db_connection()
    cur = conn.cursor()
    cur.execute("DELETE FROM user_contact WHERE user_id = %s AND contact_id = %s;", (user_id, contact_id))
    conn.commit()
    cur.close()
    conn.close()
    return 200
